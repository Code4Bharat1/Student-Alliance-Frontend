...
const CustomersPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [custRes, countRes, lastOrderRes] = await Promise.all([
          axios.get("http://localhost:5000/api/customers"),
          axios.get("http://localhost:5000/api/orders/counts/by-customer"),
          axios.get("http://localhost:5000/api/orders/last-order/by-customer"),
        ]);
        const countsMap = {};
        countRes.data.forEach((c) => {
          countsMap[c._id] = c.totalOrders;
        });
        const lastOrderMap = {};
        lastOrderRes.data.forEach((c) => {
          lastOrderMap[c._id] = c.lastOrder;
        });
        setCustomers(
          custRes.data.map((cust) => ({
            ...cust,
            totalOrders: countsMap[cust._id] || 0,
            lastOrder: lastOrderMap[cust._id] || null,
          }))
        );
      } catch (err) {
  ...
  // Helper to get status based on updatedAt
  const getStatus = (updatedAt) => {
    if (!updatedAt) return "Inactive";
    const lastUpdate = new Date(updatedAt);
    const now = new Date();
    const diffYears = (now - lastUpdate) / (1000 * 60 * 60 * 24 * 365);
    return diffYears >= 1 ? "Inactive" : "Active";
  };
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        {sortedCustomers.length > 0 ? (
          sortedCustomers.map((customer) => {
            const status = getStatus(customer.updatedAt);
            return (
              <div
                key={customer._id}
